...
@section('breadcrumbs')
    {{
       ...
       Breadcrumbs::for('add', function ($trail) {
           $trail->parent('list');
           $trail->push('カテゴリ新規登録', route('category.create'));
           })
    }}
...
@section('content')
    ...
    <div class="container-fluid">
        <form id="form-register" method="POST" action="{{ action('CategoryController@store') }}">
            @csrf
            <table class="table-form" border="1">
                ...
                <tr>
                    ...
                    <td>
                        <input id="category_name" type="text" class="qc-form-input qc-form-input-50" name="category_name" value="{{ old('category_name') }}" required>
