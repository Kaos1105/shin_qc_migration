...
@section('breadcrumbs')
    {{
       ...
       Breadcrumbs::for('promotion_circle', function ($trail, $year, $circle_name, $promotion_circle_id) {
            $trail->parent('toppage');
            $trail->push('サークル推進計画（'.$circle_name.'）（'.$year.'年）', URL::to('promotion-circle-office/'.$promotion_circle_id));
       }),
       Breadcrumbs::for('list', function ($trail, $year, $circle_name, $promotion_circle_id) {
            $trail->parent('promotion_circle', $year, $circle_name, $promotion_circle_id);
            $trail->push('サークルレベル');
       })
    }}

    {{ Breadcrumbs::render('list', $year, $circle_name, $promotion_circle_id) }}
@endsection
@section('content')
        ...
        <table class="table circle_level_list" border="1">
            ...
            @foreach($member_list as $item)
                ...
                <tr>
                    ...
                    <td>
                        {{ $item->department }}@if(isset($item->position)) ({{ $item->position }}
                        ) @endif {{ $item->name }}
                    </td>
