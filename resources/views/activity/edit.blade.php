...
@section('content')
    <div class="float-right btn-area">
        ...
        @else
            <a class="btn btn-back" href="{{ URL::to('activity/'.$activity->id.'?year='.$filterYear) }}">戻る</a>
        @endif
    </div>
    <div class="container-fluid bottom-fix">
        {{ Form::model($activity, array('route' => array('activity.update', $activity->id), 'method' => 'PUT', 'id' => 'form-register')) }}
        @csrf
        <input type="hidden" name="callback" value="{{ isset($callback)? $callback : null }}"/>
        <input type="hidden" value="{{ $filterYear }}" name="filterYear">
        <table class="table-form" border="1">
            <tr>
                <td class="td-first">{{ __('登録ID') }}</td>
                <td>
                    {{ \App\Enums\Common::show_id($activity->id) }}
                </td>
            </tr>
            <tr>
...
<!--                    <input type="hidden" value="{{ old('activity_title', isset($activity)? $activity->activity_title : null) }}"
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="qc-form-input input-s qc-datepicker" id="date_execution"
                           value="{{ old('date_execution', isset($activity->date_execution)? date("Y/m/d", strtotime($activity->date_execution)) : null) }}"
            ...
            <tr>
                ...
                <td>
                    ...
                    開始<input id="time_start" type="text" class="qc-form-input qc-form-input-10"
                             value="{{ old('time_start', isset($activity->time_start)? date("H:i", strtotime($activity->time_start)) : null) }}"
                             name="time_start" onchange="checkTime();">
                    終了<input id="time_finish" type="text" class="qc-form-input qc-form-input-10"
                             value="{{ old('time_finish', isset($activity->time_finish)? date("H:i", strtotime($activity->time_finish)) : null) }}"
                             name="time_finish" onchange="checkTime();">
                    時間　<span id="time_total">{{ \App\Enums\Common::two_number_after_comma($time) }}</span> h
                    ...
                    @if ($errors->has('time_start'))
                        <span class="qc-form-error-message">{{ $errors->first('time_start') }}</span>
                    @endif
                    @if ($errors->has('time_finish'))
                        <span class="qc-form-error-message">{{ $errors->first('time_finish') }}</span>
                    @endif
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="qc-form-input qc-form-input-50"
                           name="content1" value="{{ old('content1', isset($activity)? $activity->content1 : null) }}"/>
                </td>
            </tr>
            <tr>
...
<!--                    <textarea type="text" class="qc-form-area qc-form-input-50"
        ...
        <table class="table-form-60" border="1">
            ...
            @foreach($activity_other as $item)
                <input type="hidden" name="activity_other_id[]" value="{{ $item->id }}"/>
                <input type="hidden" name="theme_id[]" value="{{ $item->theme_id }}"/>
                <tr>
                    <td>{{ $item->theme_name }}</td>
                    <td><input type="text" name="contents[]"
                               value="{{ old('contents.'.$stt, isset($item->content)? $item->content : null) }}"
                               class="qc-form-input qc-form-input-100"/></td>
                    <td><input type="text" name="time[]"
                               value="{{ old('time.'.$stt, isset($item->time)? number_format($item->time, 2) : null) }}"
                               class="qc-form-input qc-form-input-80" id="time_{{ $item->theme_id }}"/><span
                                style="margin-left: 6px">h</span></td>
                </tr>
                <?php $stt++; ?>
            @endforeach
            @foreach($theme_list as $item)
                <input type="hidden" name="activity_other_id[]" value="0"/>
                ...
                <tr>
                    ...
                    <td><input type="text" name="time[]"
                               value="{{ old('time.'.$stt, isset($item->time)? number_format($item->time, 2) : null) }}"
                               class="qc-form-input qc-form-input-80" id="time_{{ $item->id }}"/><span
                                style="padding-left: 6px">h</span>
...
//        actitivityTitleEditor.root.innerHTML = '<?php echo old('activity_title', isset($activity)? $activity->activity_title : '') ?>' ;
    ...
    <script>          
        $(document).ready(function () {
            ...
            $("#date_intended").datepicker("option", "maxDate", $("#date_execution").val());
            $("#date_execution").datepicker("option", "minDate", $("#date_intended").val());
...
//            } else 
        ...
        function deleteFileRequestToBossActivity(fileId)
        {    
            $('#'+fileId).parent().remove();
            let existIds = $('#requestToBossFileIds').val();
            let arrExistIds = existIds.split(',');
            let arrFilterIds = arrExistIds.filter(function(value, index, arr){
                return parseInt(value) !== fileId;
            });
            $('#requestToBossFileIds').val(arrFilterIds.join());
        }
        // var extension;
        ...
        $("#btn-register").click(function () {
            ...
            let totalTime = getTotalTime();
                    @foreach($activity_other as $item)
            var effort = document.getElementById('time_{{ $item->theme_id }}').value;
            if (effort.trim() != "") {
...
//                }
                $('#content2').val(content2Editor.root.innerHTML.replace(/'/g,"\"").replace(/<p><br><\/p>/g,"").replace(/<p>/g,"<p style=" + style +">")); 
                $('#content3').val(content3Editor.root.innerHTML.replace(/'/g,"\"").replace(/<p><br><\/p>/g,"").replace(/<p>/g,"<p style=" + style +">")); 
                $('#content4').val(content4Editor.root.innerHTML.replace(/'/g,"\"").replace(/<p><br><\/p>/g,"").replace(/<p>/g,"<p style=" + style +">")); 
                $('#content5').val(content5Editor.root.innerHTML.replace(/'/g,"\"").replace(/<p><br><\/p>/g,"").replace(/<p>/g,"<p style=" + style +">")); 
